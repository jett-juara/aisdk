...
interface MultiSelectProps
	...
	/**
	 * The selected values for controlled mode.
	 */
	value?: string[];
...
export const MultiSelect = React.forwardRef<MultiSelectRef, MultiSelectProps>(
	(
		...
		}, [defaultValue, selectedValues, arraysEqual, resetOnDefaultValueChange, isControlled]);
				...
				<Popover
					...
					<PopoverContent
						...
						className={cn(
							...
							screenSize === "mobile" && "w-[var(--radix-popover-trigger-width)]",
							screenSize === "tablet" && "w-[70vw] max-w-md",
							screenSize === "desktop" && "w-[var(--radix-popover-trigger-width)]",
							popoverClassName
						)}
						style={{
						...
						align={screenSize === "mobile" ? "center" : "start"}
						...
						<Command>
							...
							<CommandList
								className={cn(
									"overflow-y-auto overflow-x-hidden multiselect-scrollbar",
									screenSize === "mobile" ? "max-h-[50vh]" : "max-h-[40vh]",
								...
								{!hideSelectAll && !searchValue && (
									<CommandGroup>
										<CommandItem
											...
											<div
												className={cn(
													"mr-2 flex h-4 w-4 items-center justify-center rounded-sm border border-text-50",
													selectedValues.length ===
														getAllOptions().filter((opt) => !opt.disabled)
															.length
														? "bg-text-50 text-background-900"
								...
								{isGroupedOptions(filteredOptions) ? (
									filteredOptions.map((group) => (
										<CommandGroup key={group.heading} heading={group.heading} className="[&_[cmdk-group-items]]:grid [&_[cmdk-group-items]]:grid-cols-1 lg:[&_[cmdk-group-items]]:grid-cols-2 [&_[cmdk-group-items]]:gap-1 [&_[cmdk-group-heading]]:col-span-2">
									...
									<CommandGroup className="[&_[cmdk-group-items]]:grid [&_[cmdk-group-items]]:grid-cols-1 lg:[&_[cmdk-group-items]]:grid-cols-2 [&_[cmdk-group-items]]:gap-1">
