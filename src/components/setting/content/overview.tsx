...
import { SettingContentProps, User, SettingStats, SystemHealth } from "@/lib/setting/types";
import {
  Users,
  UserCheck,
  Mail,
...
function QuickActions({ userRole }: { userRole: string }) {
  const actions = {
    superadmin: [
      { label: "Invite User", icon: UserPlus, href: "/setting/invitations", description: "Send invitation to new user" },
      { label: "System Health", icon: Activity, href: "/setting/system", description: "View system performance" },
      { label: "Audit Logs", icon: FileText, href: "/setting/audit", description: "Review system activity" },
      { label: "Security", icon: Shield, href: "/setting/security", description: "Manage security settings" },
    ],
    admin: [
      { label: "Manage Users", icon: Users, href: "/setting/users", description: "View and edit user accounts" },
      { label: "Permissions", icon: Lock, href: "/setting/permissions", description: "Configure user permissions" },
      { label: "System Health", icon: Activity, href: "/setting/system", description: "View system performance" },
    ],
    user: [
      { label: "My Profile", icon: Eye, href: "/setting/profile", description: "View my profile information" },
      { label: "Settings", icon: Settings, href: "/setting/settings", description: "Manage account settings" },
    ],
  };
...
function RecentActivity({ userRole }: { userRole: string }) {
  ...
  useEffect(() => {
    const fetchActivity = async () => {
      try {
        setLoading(true);
        const res = await fetch("/api/setting/recent-activity");
        if (!res.ok) {
          setLoading(false);
          return;
        }
...
function SystemStatus() {
  ...
  useEffect(() => {
    const fetchStatus = async () => {
      try {
        setLoading(true);
        const res = await fetch("/api/setting/system-status");
        if (!res.ok) {
          setLoading(false);
          return;
        }
...
/**
 * Main setting Overview Component
 */
export function SettingOverview({ user }: SettingContentProps) {
  const [stats, setStats] = useState<SettingStats | null>(null);
  const [loading, setLoading] = useState(true);
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        setLoading(true);
        const res = await fetch("/api/setting/stats");
        if (!res.ok) {
          setLoading(false);
          return;
        }
